...
function createBookmarks(bookmarks) {
    // 先将bookmarks按照JSON.parse(path).length排序
    bookmarks.sort((a, b) => {
        return JSON.parse(b.path).length - JSON.parse(a.path).length
    })
...
// 创建定时任务, 每分钟同步检测一次本地是否有变化，每5分钟强制同步一次
function openSyncFunction() {
    ...
    // 先执行一次
    if (bookmarkIsChanging || testChangeCount === 4) {
...
function getAllTabs() {
    return new Promise(async (resolve, reject) => {
        ...
        // 深拷贝bookmarksMemory
        let _bookmarks = await getBookmarks().then(bookmarks => {
            // 扁平化
            bookmarks = flattenBookmarks(bookmarks)
            return bookmarks
        })

        let bookmarksMemoryCopy = JSON.parse(JSON.stringify(_bookmarks)).filter(item => item.url)
        // 看这个url是否在书签数据中
